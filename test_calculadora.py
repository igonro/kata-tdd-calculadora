...
@pytest.mark.parametrize(
    "nums,expected",
    [([2, 2], 4), ([2.5, -6], -3.5), ([2, 2, 6, 3], 13), ([12.3], 12.3)],
)
def test_suma_devuelve_resultado_correcto(nums, expected):
    assert calc.suma(*nums) == expected
...
@pytest.mark.parametrize(
    "nums,expected",
    [([5, 3], 15), ([5.1, -3], -15.3), ([3, 2, 1, 10], 60), ([12.3], 12.3)],
)
def test_multiplicacion_devuelve_resultado_correcto(nums, expected):
    assert_almost_equal(calc.multiplicacion(*nums), expected)
...
@pytest.mark.parametrize(
    ...
    [
        ...
        ("suma -3.33", ("suma", [-3.33])),
        ("mult -3.33 1.0 8", ("multiplicacion", [-3.33, 1.0, 8])),
    ],
)
...
@pytest.mark.parametrize(
    "input,exception",
    [
        ("resta 2 4.4 5", "Error: Número de parámetros incorrecto"),
